...
src/
  ...
  index.js           # React UI, Formik form, result display
  pack-attack.css    # D&D parchment scroll theme styling
  formik.css         # Form input/button/error styling
  img/               # Scroll background image (scroll-png-26394.png)
public/
  index.html         # CRA HTML template
server.js            # Express server (production — serves /build)
build/               # Pre-built production bundle (committed to repo)
.env                 # REACT_APP_VERSION env var
Procfile             # Heroku deployment config
```

## Technology Stack

- **Frontend:** React 16, Formik 2, Yup validation, classnames
...
- **⚠️ Migration in progress:** Branch `refactor/convert-to-svelt` is converting the UI to Svelte. Do not add React-specific patterns or increase React dependencies unless explicitly asked.
...
Validation regex (Yup schema in `src/index.js`): `^[0-9]+d[0-9]+(\+[0-9]+)?$`  
Parser: `parseDice()` in `src/pack-attack.js`

## Development Commands

```bash
npm run start-dev    # Webpack dev server with hot reload (port 3000)
npm run build        # Production build output to /build
npm start            # Express server serving /build (port $PORT or 8080)
npm test             # Run Jest tests
```

## Testing

- Unit tests for dice logic live at the bottom of `src/pack-attack.js` (lines ~181–237), using Node's built-in `assert`. They run automatically when the module is loaded in a browser or Node.
- No React component tests exist. New UI logic should include Formik validation tests if added.
- Before shipping any change to `pack-attack.js`, verify the embedded tests still pass by checking the browser console for assertion errors on page load.

## Code Conventions

- Dice logic (`pack-attack.js`) is intentionally kept separate from the UI (`index.js`). Maintain this separation.
- `packAttack()` returns `[hit_count, total_damage, crit_fail_count, crit_count]` — preserve this return signature.
- Form state lives in Formik; `attack_history` is a local result accumulator used only for display.
- CSS follows BEM-adjacent naming: `.scroll_outer`, `.scroll_inner`, `.scroll_contents`, `.scroll_action`.
...
- The `/build` directory is committed to the repo and deployed directly.
- Always run `npm run build` and commit the updated `/build` before pushing a release.
- Heroku: `Procfile` → `npm start` → `server.js` (serves `/build`).
- GitHub Pages: `.github/workflows/static.yml` builds and deploys on push to main.
- Version is set in `.env` as `REACT_APP_VERSION` and displayed in the page title.
- After updating the version, bump `REACT_APP_VERSION` in `.env` and rebuild.

## What NOT to Change Without Discussion

- The crit damage formula in `attackRoll()` — it matches 5e RAW (double dice, not double total).
- The return signature of `packAttack()` — callers in `index.js` destructure all four values.
- The Yup validation regex for dice notation — changes may silently break input parsing.
